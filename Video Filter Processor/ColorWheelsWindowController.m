...
- (id)initWithWindow:(NSWindow *)window
{
    ...
    if (self) {
        [[NSNotificationCenter defaultCenter]addObserver:self
                                                selector:@selector(updateFilterFromNotification:)
                                                    name: NEW_FILTER_NOTIFICATION object:nil];
...
- (void)dealloc {
    [[NSNotificationCenter defaultCenter] removeObserver:self name:NEW_FILTER_NOTIFICATION object:nil];
...
- (void)onGainColorChange:(ColorWheel *)sender {
    ...
    filter.highlights = [self calculateHighlights];
}
- (void)onLiftColorChange:(ColorWheel *)sender {
    filter.blacks = [self calculateShadows];
}
- (void)onGammaColorChange:(ColorWheel *)sender {
    filter.mids = [self calculateMidtones];
}

- (struct RGBData)calculateHighlights{
    return [self wrapDataFromColorWheel:gainColor applyLuminance:gainSlider.floatValue];
}

- (struct RGBData)calculateMidtones{
    return [self wrapDataFromColorWheel:gammaColor applyLuminance:gammaSlider.floatValue];
}

- (struct RGBData)calculateShadows{
    return [self wrapDataFromColorWheel:liftColor applyLuminance:liftSlider.floatValue];
}
